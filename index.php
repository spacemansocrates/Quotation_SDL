...
if (isset($_SESSION['user_id']) && !$comingFromAdmin) {
    ...
    if ($_SESSION['user_role'] === 'admin') {
        header('Location: /admin/admin_dashboard.php'); // Changed to specific admin path
        exit;
    } elseif ($_SESSION['user_role'] === 'supervisor') {
        header('Location: /shop/warehouse_requests.php'); // Changed to specific supervisor path
        exit;
    } elseif ($_SESSION['user_role'] === 'manager') {
        header('Location: /shop/dashboard.php'); // Changed to specific manager path
        exit;
    } elseif ($_SESSION['user_role'] === 'staff') {
        header('Location: /
         admin/admin_quotations.php'); // Changed to specific staff path
        exit;
    } else {
        // Default redirect for any other roles not explicitly handled
        header('Location: admin_dashboard.php'); // Fallback, adjust as needed
        exit;
    }
}
                ...
                if (password_verify($password, $user['password_hash'])) {
                    ...
                    if ($user['is_active']) {
                        ...
                        if ($user['role'] === 'admin') {
                            ...
                            header('Location: shop/warehouse_requests.php');
                        } elseif ($user['role'] === 'manager') {
                            header('Location: shop/dashboard.php');
                        } elseif ($user['role'] === 'staff') {
                            header('Location: admin/admin_quotations.php');
                        } else {
                            // Fallback for any unhandled roles (e.g., if a new role is added without a specific redirect)
        ...
        * {
        ...
        body {
            ...
            background: linear-gradient(135deg, rgba(15, 23, 42, 0.8), rgba(30, 41, 59, 0.6)),
                        url('admin/images/background.JPG') center/cover no-repeat fixed;
...
<body>
    <div class="login-container">
        <div class="logo-section">
            <div class="logo">
                <img src="admin/images/logo.png" alt="Company Logo">
            </div>
